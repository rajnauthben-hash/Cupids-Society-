...
create table if not exists products (
  ...
  description text, details text, care text, featured boolean not null default false,
...
create table if not exists variants (
  ...
  created_at timestamptz not null default now(), unique (product_id, size)
);
create table if not exists product_images (
  id uuid primary key default gen_random_uuid(),
  product_id uuid not null references products(id) on delete cascade,
  url text not null, storage_path text, is_primary boolean not null default false,
...
create table if not exists orders (
  ...
  source text not null default 'cart_checkout'
    check (source in ('cart_checkout','product_page','instagram','manual','in_person')),
...
create table if not exists order_items (
  ...
  product_name text not null, size text,
  unit_price numeric(10,2) not null default 0, qty int not null default 1 check (qty>0)
);

create index if not exists idx_v_prod  on variants(product_id);
create index if not exists idx_i_prod  on product_images(product_id);
create index if not exists idx_oi_ord  on order_items(order_id);
create index if not exists idx_ord_st  on orders(status, created_at desc);
...
declare o orders; begin
  ...
  update variants v set stock=greatest(0,v.stock-i.qty)
    from order_items i where i.order_id=p_order_id and i.variant_id=v.id;
  update orders set status='confirmed',confirmed_at=now() where id=p_order_id returning * into o;
  return o; end $$;

create or replace function unconfirm_order(p_order_id uuid)
returns orders language plpgsql security definer set search_path=public as $$
declare o orders; begin
  if auth.role() is distinct from 'authenticated' then raise exception 'Not authorised'; end if;
  select * into o from orders where id=p_order_id for update;
  if not found then raise exception 'Order not found'; end if;
  if o.status<>'confirmed' then return o; end if;
  update variants v set stock=v.stock+i.qty
    from order_items i where i.order_id=p_order_id and i.variant_id=v.id;
  update orders set status='pending',confirmed_at=null where id=p_order_id returning * into o;
  return o; end $$;
...
create policy public_read on products for select to anon using (status='active');
drop policy if exists public_read on product_images;
create policy public_read on product_images for select to anon
...
create policy public_read on variants for select to anon
  using (exists (select 1 from products p where p.id=product_id and p.status='active'));
drop policy if exists public_create_pending on orders;
create policy public_create_pending on orders for insert to anon
  with check (status='pending' and source in ('cart_checkout','product_page') and confirmed_at is null);
drop policy if exists public_create_items on order_items;
create policy public_create_items on order_items for insert to anon
  with check (exists (select 1 from orders o where o.id=order_id and o.status='pending'));
...
-- Seed: 7 existing pieces. Stock is placeholder — fix real numbers in the back office.
...
select p.id, s.size, case when p.slug='thrifted-slip' then 1 else 5 end
  from products p cross join lateral (
    select unnest(case when p.slug='thrifted-slip' then array['S','M'] else array['XS','S','M','L','XL'] end) as size
  ) s
